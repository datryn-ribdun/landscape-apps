...
export default function PalsProfileInfo({
  ...
  const getActionProps = () : { icon: JSX.Element; text: string | Element; onClick?: () => void } => {
    if (isMutual || isLeech) {
      return { icon: <FaTrash className='mr-2 h-4 w-4'/>, text: 'Remove from pals', onClick: () => removePal(ship) };
    // } else if (isLeech) {
    //   return { icon: 'Clock', text: 'Pals request pending' };
    }
    if (isTarget) {
      return { icon: <FaUserPlus className='mr-2 h-4 w-4'/>, text: 'Add to pals', onClick: () => addPal(ship) };
    }

    // TODO: if %pals isn't installed, show a modal with the option to install %pals
    return { icon: <FaUserPlus className='mr-2 h-4 w-4'/>, text: 'Add to pals', onClick: handleSendRequest };
  };
    ...
    <>
      ...
      {tags.length > 0 && (
        <div className='my-1 flex flex-row flex-wrap'>
          {tags.map(t => (
            <div key={t}
              className='m-1 flex cursor-pointer rounded bg-gray-50 px-2 py-1'
              onClick={() => removeTag(t)}
            >
              {t}
              <XIcon className='ml-1 self-center' />
            </div>
          ))}
        </div>
      )}
          ...
          <form onSubmit={onSubmitTag}>
            <div className='flex shrink-0 flex-col justify-between'>
              ...
              <div className='flex flex-row'>
                <button className='button mt-2 w-20' type="submit">Add</button>
                <button className='button mt-2 ml-2 cursor-pointer' onClick={onClick}>
                  {icon}
                  {text}
                </button>
              </div>
